...
> **进度（2026-08-27）**：阶段 1（交互会话 / 三模式 / 审批门 / 成本 + 桌面建议卡片）✅；阶段 2（流式 / 上下文压缩 / 记忆 / 会话持久化恢复）✅；阶段 3（并行工具调用 / 代码索引 / 子代理 / web_search）✅ 均已提交并真实联调验证。阶段 0（提交物：README.txt + 视频 + 预演）与阶段 4（更强隔离/检查点）待做。
