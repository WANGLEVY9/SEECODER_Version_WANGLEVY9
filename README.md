...
# 整轮 Agent 任务最长 6 小时；单次模型请求和命令仍有各自更短的超时
SEECODER_TASK_TIMEOUT_S=21600
# 单轮最多 10000 次模型决策；复杂项目可显式调低，取值范围为 1-10000
...
可用 `uv run seecoder --help`、`uv run seecoder run --help` 和 `uv run seecoder chat --help` 查看参数。`--resume` 恢复已有快照，`--event-json` 输出桌面端使用的本地 JSONL 事件，`--trace-dir` 指定工作区外的审计目录，`--max-steps` 限制单次循环步数。默认 10000 步是防止大型任务仅因轮次被过早中断的安全上限；整轮默认可运行 6 小时，模型请求和命令仍分别受更短超时保护。若仍达到上限，已完成变更和会话会保留为可继续状态，而不会被呈现为工具失败。
...
- 固定底部输入框、按具体工具批次呈现的工作进展、最终 Markdown 总结、计划/工具执行轨迹和停止按钮；不会把逐 token 的模型叙述堆积为长消息。
- 运行状态、审批操作、环境信息、Git 分支与变更统计。
- Codex 风格的实时执行轨迹：仅在右侧“运行”面板展示，主会话区只保留用户消息、Agent 回答、计划卡片、审批与变更摘要；轨迹展示计划动作、准备执行的具体工具、完成或失败结果、目标路径和变更行数。
- 右侧检查器提供独立的“变更”页：显示本轮文件清单、增删行统计，并可在同一侧栏直接选择文件查看彩色只读 diff；Git 工作区使用真实 diff，非 Git 工作区使用持久化的 Agent 编辑记录和只读当前内容回退，因此不会因缺少 Git 基线而丢失变更提示。
