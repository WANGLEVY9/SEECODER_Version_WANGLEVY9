...
可用 `uv run seecoder --help`、`uv run seecoder run --help` 和 `uv run seecoder chat --help` 查看参数。`--resume` 恢复已有快照，`--event-json` 输出桌面端使用的本地 JSONL 事件，`--trace-dir` 指定工作区外的审计目录，`--max-steps` 限制单次循环步数。默认 10000 步是防止大型任务仅因轮次被过早中断的安全上限；模型请求、命令和整轮任务仍分别受超时保护。若仍达到上限，已完成变更和会话会保留为可继续状态，而不会被呈现为工具失败。
...
- 固定底部输入框、流式 Markdown 消息、计划/工具执行轨迹和停止按钮。
