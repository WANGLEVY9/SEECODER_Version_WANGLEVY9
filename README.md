...
- 17 个本地工具：`list_files`、`read_file`、`search_files`、`find_files`、`project_overview`、`write_file`、`apply_patch`、`rename_directory`、`git_diff`、`git_status`、`git_log`、`git_show`、`run_command`、`search_code`、`spawn_agent`、`web_search`、`list_skills`。
...
- `rename_directory` 通过本地文件系统 API 重命名工作区内部的代码目录，不调用 shell；不能重命名工作区根、系统目录或覆盖已有路径，在 Ask/Plan 模式中需要批准。
- `find_files`、`project_overview` 和 `git_show` 分别支持 glob 文件发现、项目清单概览与单次提交摘要，均为有界只读操作。
