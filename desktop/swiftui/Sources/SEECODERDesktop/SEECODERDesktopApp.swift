...
struct DesktopRoot: View {
  ...
  var body: some View { HStack(spacing: 0) { Sidebar(); Divider(); Conversation(); Divider(); Inspector() }.background(Color.canvas).environment(\.colorScheme, .light).preferredColorScheme(.light).sheet(isPresented: $store.showCreateWorkspace) { CreateWorkspaceSheet() } }
}

struct Sidebar: View {
  @EnvironmentObject var store: DesktopStore
  var body: some View { VStack(alignment: .leading, spacing: 14) { HStack { Image("seecoder-logo", bundle: .module).resizable().frame(width: 27, height: 27).clipShape(RoundedRectangle(cornerRadius: 8)); Text("SEECODER").font(.system(size: 16, weight: .bold)).foregroundStyle(Color.ink); Spacer() }.padding(.bottom, 16); Button(action: store.newSession) { Label("新对话", systemImage: "square.and.pencil") }.buttonStyle(SidebarAction()); Button(action: store.chooseWorkspace) { Label("打开工作区", systemImage: "folder") }.buttonStyle(SidebarAction()); Button(action: store.openCreateWorkspace) { Label("新建工作区", systemImage: "folder.badge.plus") }.buttonStyle(SidebarAction()); Text("项目会话").font(.caption.weight(.semibold)).foregroundStyle(Color.muted).padding(.top, 18); ScrollView { LazyVStack(spacing: 4) { ForEach(store.sessions) { session in Button { store.select(session) } label: { VStack(alignment: .leading, spacing: 4) { Text(session.title).lineLimit(1).font(.system(size: 13, weight: .semibold)).foregroundStyle(Color.ink); Text(session.workspace.isEmpty ? "未选择工作区" : URL(fileURLWithPath: session.workspace).lastPathComponent).lineLimit(1).font(.caption).foregroundStyle(Color.muted) }.frame(maxWidth: .infinity, alignment: .leading).padding(9).background(store.selectedID == session.id ? Color.blue.opacity(0.10) : .clear, in: RoundedRectangle(cornerRadius: 8)) }.buttonStyle(.plain) } } }; Spacer(); Divider(); Label("本地优先", systemImage: "checkmark.circle.fill").font(.caption.weight(.semibold)).foregroundStyle(.green); Text("会话仅保存在此设备\n不会保存 API key").font(.caption2).foregroundStyle(Color.muted) }.padding(18).frame(width: 250).background(Color.sidebar) }
}
...
struct Onboarding: View { @EnvironmentObject var store: DesktopStore; var body: some View { VStack(spacing: 14) { Image("seecoder-logo", bundle: .module).resizable().frame(width: 64, height: 64).clipShape(RoundedRectangle(cornerRadius: 20)); Text("选择一个开发区域").font(.system(size: 32, weight: .bold)).foregroundStyle(Color.ink); Text("选择已有本地文件夹，或创建一个新的会话工作区。\n所有本地操作都会限制在你选定的目录中。").multilineTextAlignment(.center).foregroundStyle(Color.muted); HStack { Button("选择本地文件夹", action: store.chooseWorkspace).buttonStyle(.borderedProminent); Button("新建会话工作区", action: store.openCreateWorkspace).buttonStyle(.bordered) } }.frame(maxWidth: .infinity, minHeight: 480).padding(.bottom, 70) } }
...
struct Composer: View { @EnvironmentObject var store: DesktopStore; var body: some View { VStack(spacing: 8) { HStack { TextEditor(text: $store.draft).font(.system(size: 14)).foregroundStyle(Color.ink).scrollContentBackground(.hidden).background(Color.white).frame(minHeight: 68).disabled(!store.hasWorkspace || store.isRunning).overlay(alignment: .topLeading) { if store.draft.isEmpty { Text(store.hasWorkspace ? "描述一个真实的编程任务…" : "先选择或创建一个本地开发区域…").foregroundStyle(Color.muted).padding(.top, 8).padding(.leading, 5).allowsHitTesting(false) } }; Button(action: store.send) { Image(systemName: "arrow.up") }.buttonStyle(.borderedProminent).disabled(!store.hasWorkspace || store.isRunning) }; HStack { Picker("模式", selection: $store.mode) { Text("询问").tag("ask"); Text("计划").tag("plan"); Text("自动").tag("auto") }.labelsHidden().frame(width: 86); Text("本地 · 受限执行").font(.caption).foregroundStyle(Color.muted); Spacer(); if store.isRunning { Button("停止", action: store.stop).buttonStyle(.bordered) } } }.padding(12).background(.white, in: RoundedRectangle(cornerRadius: 14)).overlay(RoundedRectangle(cornerRadius: 14).stroke(Color.line, lineWidth: 1)).padding(.horizontal, 42).padding(.bottom, 14) } }
...
extension Color { static let canvas = Color(red: 0.985, green: 0.978, blue: 0.965); static let sidebar = Color(red: 0.958, green: 0.965, blue: 0.965); static let inspector = Color(red: 0.975, green: 0.969, blue: 0.957); static let line = Color(red: 0.86, green: 0.89, blue: 0.89); static let ink = Color(red: 0.10, green: 0.18, blue: 0.23); static let muted = Color(red: 0.35, green: 0.45, blue: 0.51) }
