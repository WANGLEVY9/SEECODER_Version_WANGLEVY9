...
struct Composer: View { @EnvironmentObject var store: DesktopStore; var body: some View { VStack(spacing: 8) { HStack(alignment: .bottom, spacing: 10) { TextEditor(text: $store.draft).font(.system(size: 14)).foregroundStyle(Color.ink).scrollContentBackground(.hidden).background(Color.white).frame(height: 68).disabled(!store.hasWorkspace || store.isRunning).overlay(alignment: .topLeading) { if store.draft.isEmpty { Text(store.hasWorkspace ? "描述一个真实的编程任务…" : "先选择或创建一个本地开发区域…").foregroundStyle(Color.muted).padding(.top, 8).padding(.leading, 5).allowsHitTesting(false) } }; Button(action: store.send) { Image(systemName: "arrow.up") }.buttonStyle(.borderedProminent).disabled(!store.hasWorkspace || store.isRunning) }; HStack { Picker("模式", selection: $store.mode) { Text("询问").tag("ask"); Text("计划").tag("plan"); Text("自动").tag("auto") }.labelsHidden().frame(width: 80); Text("本地 · 受限执行").font(.caption).foregroundStyle(Color.muted); Spacer(); if store.isRunning { Button("停止", action: store.stop).buttonStyle(.bordered) } } }.padding(11).frame(maxWidth: 720).background(.white, in: RoundedRectangle(cornerRadius: 13)).overlay(RoundedRectangle(cornerRadius: 13).stroke(Color.line, lineWidth: 1)).frame(maxWidth: .infinity).padding(.horizontal, 24).padding(.bottom, 12) } }
